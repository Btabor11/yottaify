...
/**
 * Sheet 04 — the form, with the specification of what a slot actually is
...
export function ReserveBay() {
  return (
    <section id="reserve" className="relative scroll-mt-24 border-t border-[var(--rule-strong)] bg-[var(--surface)] py-16 md:py-24">
      ...
      <div className="d3-shell relative">
        ...
        <div className="grid gap-x-10 gap-y-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,17rem)]">
          ...
          <aside className="flex flex-col gap-8 lg:sticky lg:top-24 lg:self-start">
            <div>
              <p className="d3-tag border-b border-[var(--rule-strong)] pb-2 text-[var(--ink-3)]">What a slot is</p>
              <dl className="mt-3">
                {[
                  ...
                  <div key={k} className="flex items-baseline justify-between gap-3 border-b border-[var(--rule)] py-2.5 last:border-0">
            ...
            <div>
              <p className="d3-tag border-b border-[var(--rule-strong)] pb-2 text-[var(--ink-3)]">Then what</p>
              <ol className="mt-3">
                {FORM_COPY.whatHappensNext.map((step, i) => (
                  <li key={step} className="grid grid-cols-[1.25rem_1fr] gap-2 border-b border-[var(--rule)] py-2.5 last:border-0">
                    <span className="d3-figure text-[0.625rem] text-[var(--accent)]">{String(i + 1).padStart(2, "0")}</span>
                    <span className="d3-body text-[0.75rem] leading-snug text-[var(--ink-2)] text-pretty">{step}</span>
                  </li>
                ))}
              </ol>
            </div>
            ...
            <div className="border-l-2 border-[var(--caution)] pl-4">
              ...
              <p className="d3-voice mt-3 text-[1.125rem] leading-[1.25] text-[var(--ink)] text-pretty md:mt-2">
