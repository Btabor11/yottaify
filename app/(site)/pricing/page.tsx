          ...
          <div className="d3-shell relative pb-14 pt-28 md:pb-20 md:pt-36">
            <dl className="d3-titleblock d3-rise" style={{ ["--cols" as string]: 4 }}>
              ...
              <div>
                ...
                <dd className="d3-figure mt-1 text-[1rem] leading-[1.15] text-[var(--ink-2)] md:text-[1.125rem] md:leading-none">{QUOTE.unit}</dd>
              </div>
            </dl>
            ...
            {/* The two market numbers up front, and our position beside them.
                Ours is a sentence rather than a figure on purpose — see QUOTE. */}
            <dl
              className="d3-rise mt-11 grid gap-px border border-[var(--rule-strong)] bg-[var(--rule)] sm:grid-cols-3"
              style={{ animationDelay: "240ms" }}
            >
              {[
                { label: "Lowest verified in stock", value: verified.display, sub: QUOTE.unit, tone: "var(--ink)" },
                { label: "Median across tracked providers", value: median.display, sub: QUOTE.unit, tone: "var(--ink)" },
              ].map((cell) => (
                <div key={cell.label} className="bg-[var(--surface)] px-5 py-5">
                  ...
                  <dd className="d3-tag mt-2.5 text-[0.4375rem] text-[var(--ink-3)]">{cell.sub}</dd>
                </div>
              ))}
              <div className="bg-[var(--surface)] px-5 py-5">
                <dt className="d3-tag text-[0.5rem] text-[var(--accent)]">Our on-demand quote</dt>
                <dd className="d3-display mt-3 max-w-[9em] text-[clamp(1.125rem,2.2vw,1.5rem)] leading-[1.1] text-balance text-[var(--accent)]">
                  {QUOTE.position}
                </dd>
                <dd className="d3-tag mt-2.5 text-[0.4375rem] text-[var(--ink-3)]">
                  {QUOTE.label} · {QUOTE.unit}
                </dd>
              </div>
            </dl>
            ...
            <div className="d3-rise mt-6 border-l-2 border-[var(--caution)] pl-4" style={{ animationDelay: "300ms" }}>
              ...
              <p className="d3-body mt-2 max-w-[70ch] text-[0.875rem] text-[var(--ink-2)] text-pretty">
                ...
                actually orderable, and that is the comparison we think is honest — so it is the one
                we hold our own quote against.
              </p>
            </div>
          </div>
        </header>
        ...
        <section className="relative border-y border-[var(--rule-strong)] bg-[var(--surface)]">
          ...
          <div className="d3-shell relative grid gap-x-14 gap-y-8 py-14 md:py-20 lg:grid-cols-[minmax(0,1fr)_minmax(0,32rem)]">
            ...
            <div className="space-y-5" data-r-group>
              ...
              <p className="d3-display text-[1.5rem] text-[var(--ink)]" style={{ ["--wght" as string]: 720 }}>
                {CONTRACT.inquiry.heading}
              </p>
              <p className="d3-body text-[0.9375rem] text-[var(--ink-2)] text-pretty">{CONTRACT.inquiry.body}</p>
              <p className="d3-body border-l-2 border-[var(--caution)] pl-4 text-[0.8125rem] text-[var(--ink-3)] text-pretty">
